...
class SecureApp:
    ...
    def __init__(self):
        ...
        self.audit_logger = AuditLogger(LOG_FILE, self.db_manager)
    ...
    def show_password_dialog(self, title: str, prompt: str) -> str:
        ...
        show_password_checkbox = ctk.CTkCheckBox(
            ...
            command=lambda: self.toggle_dialog_password_visibility(password_entry, show_password_var),
            width=50
        )
    ...
    def create_file_management_tab(self):
        ...
        upload_label = ctk.CTkLabel(upload_frame, text="Upload File", font=ctk.CTkFont(size=16, weight="bold"))
        ...
        list_label = ctk.CTkLabel(list_frame, text="All Files" if self.current_user.role == "admin" else "Your Files", font=ctk.CTkFont(size=16, weight="bold"))
        list_label.pack(pady=10)
        
        # File list treeview
        if self.current_user.role == "admin":
            columns = ("ID", "Filename", "Owner", "Size", "Created", "Last Accessed", "Access Count")
        else:
            columns = ("ID", "Filename", "Size", "Created", "Last Accessed", "Access Count")
    ...
    def create_user_management_tab(self):
        ...
        create_label = ctk.CTkLabel(create_frame, text="Create New User", font=ctk.CTkFont(size=16, weight="bold"))
        ...
        # Username
        ctk.CTkLabel(form_frame, text="Username:").grid(row=0, column=0, padx=10, pady=5, sticky="w")
        ...
        # Password
        ctk.CTkLabel(form_frame, text="Password:").grid(row=2, column=0, padx=10, pady=5, sticky="w")
        ...
        # Role
        ctk.CTkLabel(form_frame, text="Role:").grid(row=3, column=0, padx=10, pady=5, sticky="w")
        self.new_role_var = tk.StringVar(value="user")
        role_combo = ctk.CTkComboBox(form_frame, values=["user", "admin", "readonly"], variable=self.new_role_var, width=200)
    ...
    def handle_file_upload(self):
        ...
        password = self.show_password_dialog("File Encryption", "Enter your password for file encryption:")
        if not password:
            return
        
        # Upload file
        success, message = self.file_manager.upload_file(file_path, self.current_user.username, password)
    ...
    def handle_file_download(self):
        ...
        password = self.show_password_dialog("File Decryption", "Enter your password for file decryption:")
        if not password:
            return
    ...
    def handle_file_delete(self):
        ...
        if messagebox.askyesno("Confirm", "Are you sure you want to delete this file?"):
            ...
            success, message = self.file_manager.delete_file(file_id, self.current_user.username)
        ...
        for file_info in files:
            if self.current_user.role == "admin":
                self.file_tree.insert("", "end", values=(
                    ...
                    file_info['last_accessed'].strftime("%Y-%m-%d %H:%M") if file_info['last_accessed'] else "Never",
    ...
    def refresh_audit_log(self, log_text):
        ...
        for event in events:
            log_entry = f"{event['timestamp']} - {event['username']} - {event['action']} - {event['resource']} - {'SUCCESS' if event['success'] else 'FAILED'}\n"
